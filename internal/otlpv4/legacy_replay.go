...
// ReplayLegacyV3 reads a cleanly closed v0.3.0 database without modifying it
// and emits deterministic normalized envelopes. The signal order is logs,
// traces, then metrics; v0.3.0 did not retain cross-signal ingest order.
func ReplayLegacyV3(ctx context.Context, dataRoot string, fn func(Envelope) error) error {
	...
	return replayLegacyMetrics(ctx, filepath.Join(dataRoot, "metrics"), fn)
}
...
func replayLegacyEventStore(ctx context.Context, dir, signal string, fn func(Envelope) error) error {
	...
	for _, segment := range segments {
		...
		if err := replayLegacySegment(ctx, filepath.Join(dir, segment.FileName), signal, fn); err != nil {
			return err
		}
	}
	return nil
...
func replayLegacyMetrics(ctx context.Context, dir string, fn func(Envelope) error) error {
	...
	payload, err := readRegular(filepath.Join(dir, "manifest.json"))
	if errors.Is(err, os.ErrNotExist) {
