...
func rebuildCatalogFromManifest(dir string, manifest Manifest) (Catalog, error) {
	...
	for _, meta := range manifest.Blocks {
		if meta.Kind == BlockKindHistogram {
			directory, err := histogram.ReadDirectory(filepath.Join(dir, meta.Path))
			if err != nil {
				return Catalog{}, err
			}
...
// reconcileLastTouchFromBlocks updates registry last-touch state from sealed
// blocks.
// It also imports series that are present in block directories but missing from
// the catalog, which can happen if a REGISTER was lost before the last catalog
